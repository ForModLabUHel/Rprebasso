...
calBioIndices <- function(modOut){
  ...
  if(class(modOut) == "multiPrebas"){
    ...
    Vtot = apply(modOut$multiOut[,,30,,1],1:2,sum) 
    ...
    ageoldest = apply(modOut$multiOut[,,7,,1],1:2,max)
    Nstems = apply(modOut$multiOut[,,17,,1],1:2,sum)
    BAtot = apply(modOut$multiOut[,,13,,1],1:2,sum) 
    ...
    Vtot = apply(PREBASout$output[,30,,1],1,sum)
    Ppine = Vpine / Vtot *100 
    Pspruce = Vspruce / Vtot *100
    Pdec = Vdec / Vtot *100
    ageoldest = apply(PREBASout$output[,7,,1],1,max)
    Ppine[which(is.na(Ppine))] <- 0
    Pspruce[which(is.na(Pspruce))] <- 0
    Pdec[which(is.na(Pdec))] <- 0
    Nstems = apply(PREBASout$output[,17,,1],1,sum)
    BAtot = apply(PREBASout$output[,13,,1],1,sum) 
