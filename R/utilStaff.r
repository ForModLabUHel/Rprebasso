###default parameters
pCROB <- matrix(NA,41,4,dimnames = list(NULL,c("pisy","piab","beal","eugl")))
# pCROB <- matrix(NA,41,3,dimnames = list(NULL,c("pisy","piab","beal")))
...
pCROB[,4] <- c(0.243295313,225.6258261,26.32261082,0.256805073,1.962276167,0.40277937,0.203183449,0.202104963,0.472640371,0.021649125,1.829826494,
               0.010254229,0.295897229,0.237566027,299.7931808,0.015134211,1372.570582,0.34420028,0.989050234,0.986097453,0.101577929,0.287938712,
               0.693715072,0.848556233,0.87642276,1,1,2,3.1,5800,0.046389412,39.99412396,0.031339998,0.000274012,-3.324,0.135,-0.03,0.036842852,
               40.60910852,2.789145275,1.134358873)

pHcM <- matrix(NA,6,4,dimnames = list(NULL,c("pisy","piab","beal","eugl")))
...
pHcM[,4] <- c(-1.9287089,1.0760549,-0.1079130,0.1922377,0.1363654,-0.3804504)
...
parsAWEN <- matrix(NA,12,4,dimnames = list(NULL,c("pisy","piab","beal","eugl")))
...
litterSizeDef <- matrix(NA,3,4,dimnames = list(c("CwoodLit","FwoodLit","NwoodLit"),c("pisy","piab","beal","eugl")))
litterSizeDef[,1] <- c(30,2,0)
litterSizeDef[,2] <- c(30,2,0)
litterSizeDef[,3] <- c(10,2,0)
litterSizeDef[,4] <- c(20,2,0)
