...
if(rndm <= wrisk) outDist(year,7) = 1 !wind disturbance occurs
if(rndm > wrisk) outDist(year,7) = 0 !... or doesn't.

!step 2: severity class (for now with shares/probabilities from post-storm inventory )
call random_number(rndm)
outDist(year,8) = 1 !set sevclass to 1 
if(rndm <= 0.13888889) outDist(year,8) = 2 
if(rndm <= 0.05555556) outDist(year,8) = 3 !these are now sampled even if there's no disturbance occuring; keep for dev purposes, only calculate when dist occurres in final version

! step 3: sample from severity class-specific set of relative disturbed volumes
if(outDist(year,7)==1) then ! wind disturbance occurs
   if (outDist(year,8)==1) then ! sevclass 1
     ...
     outDist(year,9) = sc1vols(distvloc)
