...
integer, intent(in) :: nYears(nSites),nLayers(nSites),protect
 ...
 real (kind=8), intent(inout) :: siteInfo(nSites,10),thdPer(nSites),limPer(nSites), latitude(nSites)
...
do i = 1,nSites
 do ijj = 1,nLayers(i)
	...
	call initBiomasses(pCrobas(:,species),initVar(i,:,ijj),siteInfo(i,3),multiOut(i,1,:,ijj,1),multiOut(i,1,3,ijj,1))
 enddo
enddo
 ...
 ! write(1,*) i
	...
	! nYears(i) = nYears(i)
	  call prebas(nYears(i),nLayers(i),allSP,siteInfo(i,:),pCrobas,initVar(i,:,1:nLayers(i)),&
		...
		fertThin,flagFert,nYearsFert,protect,mortModX,ECMmod,pECMmod,ETSstart(climID),latitude(i)) !energyCut)
