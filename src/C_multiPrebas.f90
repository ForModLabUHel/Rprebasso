...
subroutine multiPrebas(multiOut,nSites,nClimID,nLayers,maxYears,maxThin, &
		...
		defaultThin,ClCut,energyCuts,inDclct,inAclct,dailyPRELES,yassoRun,multiEnergyWood,tapioPars) !!energCut
 ...
 ! write(1,*) i
	...
	! nYears(i) = nYears(i)
	  call prebas(nYears(i),nLayers(i),allSP,siteInfo(i,:),pCrobas,initVar(i,:,1:nLayers(i)),&
		...
		multiEnergyWood(i,1:nYears(i),1:nLayers(i),:),tapioPars) !energyCut)
