...
integer :: yearMan_GV,CO2model, etmodel, gvRun, fertThin, ECMmod, oldLayer !not direct inputs anymore, but in prebasFlags fvec
...
!set species from thinning matrix (end)
  ...
  ! if(year >= yearX) then
   ...
   !! calculate litter including residuals from thinned trees
    !energyCut
     S_fol = wf_STKG + stand_all(26,ij)
     S_fr = W_froot + stand_all(27,ij)
     if(energyCut==1.) then
      energyWood(year,ij,2) = (W_branch + W_croot*0.3 + W_stem* (1-harvRatio)) * energyRatio
      species = int(max(1.,stand(4)))
  if(pCrobas(2,species)>0.) energyWood(year,ij,1) = energyWood(year,ij,2) / pCrobas(2,species)
      S_branch = max(0.,((W_branch) * (1-energyRatio) + stand_all(28,ij) + Wdb + &
          W_stem* (1-harvRatio)* (1-energyRatio) + &
          (0.3 * (1-energyRatio)+0.7) * W_croot *0.83))
      S_wood = stand_all(29,ij) + (0.3 * (1-energyRatio)+0.7) * W_croot *0.17
     else
      S_branch = max(0.,(W_branch + Wdb + W_croot*0.83 + stand_all(28,ij) + W_stem* (1-harvRatio)))
      S_wood = stand_all(29,ij) + W_croot*0.17!(1-harvRatio) takes into account of the stem residuals after thinnings
     endif
    !energyCut
     STAND(26) = S_fol
     STAND(27) = S_fr
...
! if(siteType==0.) siteType = siteInfo(3)
  if(yearMan_GV==1) then
  ! yearMan_GV = 0
      call fAPARgv(0.99d0, ETSmean, siteInfo(3), fAPARgvX, GVout(year,2), &
...
!!!respiration factor takes into account of extra respiration from harvested trees
      if(yearMan_GV==1) then
		resp_factor = (outt(13,ij,2) + BA)/ BA
	  else
		resp_factor = 1.
	  endif
      Respi_tot = (gpp_sp - npp) *resp_factor
